...
namespace Accounts_ControlModules
{
    class Control_Modules
    {
        ...
        public long DateDiff(DateInterval intervalType, System.DateTime dateOne, System.DateTime dateTwo)
        {
            switch (intervalType)
            {
        ...
        public void sendEMailThroughOUTLOOK_Send(String toid, String ccid, String subject, String Body, params String[] FilePath)
        {
            try
            {
                Int32 ArrayIndex = 0;
                ...
                // oMsg.HTMLBody = oMsg.Body;
                String Text = "<html><body><basefont size=4 font face= courier new>" + " " + "</font></body></html>";
                Text = Text + "<Br>";
                Text = Text + "Dear Sir, ";
                Text = Text + "<Br>";
                Text = Text + "         Please Find attachment ";
                Text = Text + "<Br>";
                Text = Text + "<Br>";
                Text = Text + "<Br>";
                Text = Text + "<Br>";
                Text = Text + "This is System Generated Mail.";
                oMsg.HTMLBody = Text;
        ...
        public string[] SplitByLenght(string s, int split)
        {
        ...
        public DateTime GetServerDateTime_Local()
        {
            DateTime Dt = DateTime.Now;
            try
            {
        ...
        public String SumWithCondtion_WONull(ref MyDataGridView DGV, String ColumnName, String CheckColName, String Value, params String[] Condition_NotNullColumns)
        {
            Decimal SumValue = 0;
            try
            {
                for (int i = 0; i <= DGV.Rows.Count - 1; i++)
                {
                    if (Check_EmptyinDataGridView(ref DGV, i, Condition_NotNullColumns) != true)
                    {
                        ...
                        {
                            if (DGV[ColumnName, i].Value.ToString() != String.Empty)
                            {
        ...
        public String View_Details(String TblName, String Condition)
        {
            ...
            try
            {
                ...
                if (Dt.Rows.Count > 0)
                {
                    ...
                    {
                        User = GetData_InString("Projects_User_Master", "User_Code", User, "User_Name");
                    }
            ...
            try
            {
                ...
                foreach (DataColumn DC in NewDt.Columns)
                {
                    ...
                    if (Convert.ToString(DC.DataType) == "System.Double" || Convert.ToString(DC.DataType) == "System.Int32" || Convert.ToString(DC.DataType) == "System.Decimal")
                    {
                        ...
                        if (DC.ColumnName.ToUpper() == "DEBIT" || DC.ColumnName.ToUpper() == "CREDIT" || DC.ColumnName.ToUpper() == "PAYMENT" || DC.ColumnName.ToUpper() == "RECEIPT" || DC.ColumnName.ToUpper() == "AMOUNT" || DC.ColumnName.ToUpper().Contains("AMOUNT") || DC.ColumnName.ToUpper().Contains("VALUE") || DC.ColumnName.ToUpper().Contains("QTY"))
                        {
                            Flag = true;
                            break;
                        }
        ...
        public System.Data.DataTable V_DataTable_TribleDigit(ref System.Data.DataTable Dt)
        {
            Boolean Flag = false;
            System.Data.DataTable NewDt;
            try
                ...
                if (Flag == true)
                {
                    ...
                    foreach (DataColumn DC in NewDt.Columns)
                    {
                        if (Convert.ToString(DC.DataType) == "System.Double")
                        {
                            if (DC.ColumnName.ToUpper() != "CLBAL" && DC.ColumnName.ToUpper() != "BILLNO" && DC.ColumnName.ToUpper() != "MBILL" && DC.ColumnName.ToUpper() != "BILL_NO" && DC.ColumnName.ToUpper() != "LOCATION" && DC.ColumnName.ToUpper() != "MINNO" && DC.ColumnName.ToUpper() != "MAXNO" && DC.ColumnName.ToUpper() != "GSNNO" && DC.ColumnName.ToUpper() != "ACCNO" && DC.ColumnName.ToUpper() != "SLNO" && DC.ColumnName.ToUpper() != "TAX_PER" && DC.ColumnName.ToUpper() != "SRNO" && DC.ColumnName.ToUpper() != "DCNO" && DC.ColumnName.ToUpper() != "RECNO" && DC.ColumnName.ToUpper() != "EC" && DC.ColumnName.ToUpper() != "E_NO" && DC.ColumnName.ToUpper() != "SALESQTY %" && DC.ColumnName.ToUpper() != "SALESMTR %" && DC.ColumnName.ToUpper() != "GRN_NO")
                            {
                                Dr[DC] = Convert.ToDouble(Sum_Trible(ref NewDt, DC.ColumnName, true));
                            }
                            if (Dr[DC] != DBNull.Value)
                            {
                                if (Convert.ToDouble(Dr[DC]) == 0)
        ...
        bool Connection_Initialize()
        {
            try
            {
                ...
                //    SR.Close();
                //}
                if (System.IO.File.Exists("C:\\Vaahrep\\Projects.txt") == true)
                {
                    SR = new StreamReader("C:\\Vaahrep\\Projects.txt");
                    ConnStr = Connection_Ascii_Reverse(SR.ReadToEnd().Trim());
                    SR.Close();
                }
                else
        ...
        bool SqlConnection_Initialize()
        {
            try
            {
                ...
                //    SR.Close();
                //}
                if (System.IO.File.Exists("C:\\Vaahrep\\ProjectsSql.txt") == true)
                {
                    SR = new StreamReader("C:\\Vaahrep\\ProjectsSql.txt");
                    ConnStr = Connection_Ascii_Reverse(SR.ReadToEnd().Trim());
                    ConnStr += " ; Max Pool Size =2000;";
                    SR.Close();
                }
        ...
        public void Cn_Open()
        {
            try
            {
                if (Cn.State == ConnectionState.Closed)
                {
                    ...
                    Cn.Open();
                    Close_Existing_Sessions_Odbc_Prg();
                }
            }
            catch (Exception ex)
            {
        ...
        void Close_Existing_Sessions_Odbc_Prg()
        {
            try
            {
                //OdbcCommand Cmd = new OdbcCommand("Declare @Str as varchar (100); Set @Str = HOST_NAME(); Declare @PrgName as Varchar (50) = 'VSocks'; Exec Vaahini_ERP_Gainup.dbo.Clear_All_InActive_Sessions_Host_Name_Odbc_Prgname @Str, @PrgName;", Cn);
                OdbcCommand Cmd = new OdbcCommand("Declare @Str as varchar (100); Set @Str = HOST_NAME(); Declare @PrgName as Varchar (50) = 'Projects'; Exec Vaahini_ERP_Gainup.dbo.Clear_All_InActive_Sessions_Host_Name_Odbc_Prgname @Str, @PrgName;", Cn);
                Cmd.ExecuteNonQuery();
            }
            catch (Exception ex)
            {
        ...
        void Close_Existing_Sessions()
        {
            try
            {
                SqlCommand Cmd = new SqlCommand("Declare @Str as varchar (100); Set @Str = HOST_NAME(); Exec Vaahini_ERP_Gainup.dbo.Clear_All_InActive_Sessions_Host_Name @Str;", SqlCn);
                Cmd.ExecuteNonQuery();
            }
            catch (Exception ex)
            {
        ...
        public Boolean EntryDate_Check(DateTime EntryDate)
        {
            try
            {
                DataTable DtT = new DataTable();
                Load_Data("Select DateDiff(dd, Cast(Getdate() As Date), Cast('" + String.Format("{0:dd-MMM-yyyy}", EntryDate) + "'As Date))Diff", ref DtT);
                if (DtT.Rows.Count == 0 || Convert.ToInt64(DtT.Rows[0]["Diff"].ToString()) > 0)
                {
                    return false;
                }
                else
        ...
        public String Sum_With_Digits(ref MyDataGridView DGV, String ColumnName, int n, params String[] Condition_NotNullColumns)
        {
            Decimal SumValue = 0;
            try
            {
                ...
                return String.Format("{" + F + "}", SumValue);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public String Company_Address(int CompCode)
        {
            ...
            try
            {
                ...
                SW.WriteLine(GetData_InString("Projects_Companymas", "CompCode", CompCode.ToString(), "Compaddress"));
                SW.Close();
        ...
        public Boolean Check_Table(String TblName)
        {
            try
            {
                ...
                OdbcCommand Cmd = new OdbcCommand("select * from projects.sys.SysObjects where Name = '" + TblName.ToUpper() + "' and Xtype = 'U'", Cn);
                OdbcDataReader Rd = Cmd.ExecuteReader();
                if (Rd.HasRows == true)
                {
                    Flag = true;
        ...
        public Int64 Get_RecordCount_WO_CheckTable (String ObjName, String Condition)
        {
            try
            {
                Int64 Count = 0;
        ...
        String Menu_underName(String Under)
        {
            ...
            try
            {
                Code = GetData_InString("Projects.dbo.Projects_Menu_Master_New", "Menu_name", Under.ToUpper(), "Menu_CName");
                if (Code == string.Empty)
                {
                    Code = "Main";
                }
        ...
        public void Menu_save(String MenuName, String MenuCName, String Under)
        {
            try
            {
                Execute("Insert into Projects.dbo.Projects_Menu_Master_New values ('" + MenuName.ToUpper() + "','" + MenuCName.ToUpper() + "', '" + Menu_underName(Under) + "')");
            }
            catch (Exception ex)
            {
                throw ex;
